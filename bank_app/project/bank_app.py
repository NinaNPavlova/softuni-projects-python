...
class BankApp:
    ...
    def add_loan(self, loan_type: str):
        if loan_type not in self.LOAN_TYPES:
            raise "Invalid loan type!"
        self.loans.append(self.LOAN_TYPES[loan_type]())
        return f"{loan_type} was successfully added."

    def add_client(self, client_type: str, client_name: str, income: float):
        if client_type not in self.CLIENT_TYPES:
            raise "Invalid client type!"
        if len(self.clients) >= self.capacity:
            raise "Not enough bank capacity."

        new_client = self.CLIENT_TYPES[client_type](client_name, client_type, income)
